...
def _validate_box(box: Dict) -> None:
    ...
    if "combined_title" in box:
        combined_title = box["combined_title"]
        if combined_title is not None and (not isinstance(combined_title, str) or not combined_title.strip()):
            raise BoxConfigError("Box 'combined_title' must be a non-empty string or None")
...
def _normalize_box(box: Dict) -> Dict:
    normalized = {
        ...
        "no_blank":      bool(box.get("no_blank", False)),
        "group":         str(box["group"]).strip() if box.get("group") is not None else None,
        "combined_title": str(box["combined_title"]).strip() if box.get("combined_title") is not None else None,
        "is_answerer":   bool(box.get("is_answerer", False)),
    }
...
class BoxConfigService:
    ...
    def set_combined_title(self, title: str, combined_title: Optional[str]) -> Dict:
        return self.update_field(title, combined_title=combined_title)
